...
class FNO1d(nn.Module):
    def __init__(self, modes, width):
        ...
        self.fc2 = nn.Linear(128, sstorey)
...
y1_train = y1_train.reshape(ntrain,stime,y1_train.shape[-1])
y1_test = y1_test.reshape(ntest,stime,y1_train.shape[-1])
...
for ep in range(epochs):
    ...
    for x, y in train_loader:
        ...
        out1 = model(x)
        loss = myloss(out1.view(batch_size_train,-1), y1.view(batch_size_train,-1))
        loss.backward()
...
batch_size_test = 1
test_loader = torch.utils.data.DataLoader(torch.utils.data.TensorDataset(x_test, y1_test), batch_size=batch_size_test, shuffle=False)    
pred_u1 = torch.zeros(num_test,stime,y1_test.shape[-1])
