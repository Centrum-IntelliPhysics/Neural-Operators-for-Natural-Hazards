...
class FNO1d(nn.Module):
    def __init__(self, modes, width):
        ...
        self.fc0 = nn.Linear(39, self.width) # input channel is 2: (a(x), x)
        ...
        self.fc2 = nn.Linear(32, 1) # 6 to account for 6 floors
...
reader = MatReader('./Results/pred_deeponet_train')
x1_train = reader.read_field('ground_motion')
x2_train = reader.read_field('disp_pred')
y_train = reader.read_field('disp_target')

reader = MatReader('./Results/pred_deeponet_test')
x1_test = reader.read_field('ground_motion')
x2_test = reader.read_field('disp_pred')
y_test = reader.read_field('disp_target')

s = y_train.shape[1]
ntrain = y_train.shape[0]
ntest = y_test.shape[0]

x1_train = x1_train.reshape(ntrain,s,37)
x2_train = x2_train.reshape(ntrain,s,1)
x_train = torch.concatenate((x1_train, x2_train), axis = -1)

x1_test = x1_test.reshape(ntest,s,37)
x2_test = x2_test.reshape(ntest,s,1)
x_test = torch.concatenate((x1_test, x2_test), axis = -1)
...
np.savetxt(save_results_to+'/train_error_fno.txt', train_error)
np.savetxt(save_results_to+'/test_error_fno.txt', test_error)    
save_models_to = save_results_to +"model/"
if not os.path.exists(save_models_to):
    os.makedirs(save_models_to)
    
torch.save(model, save_models_to+'Wind_fno')
...
scipy.io.savemat(save_results_to+'Wind_responses_test_fno.mat', 
...
fig.savefig(save_results_to+'loss_history_fno.png')
