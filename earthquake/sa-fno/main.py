...
class PARAM(nn.Module):
    def __init__(self, dim):
        ...
        self.lam = nn.Parameter(torch.randn(dim,6))
        #self.lam = nn.Parameter(torch.ones(dim,6))
...
################################################################
reader = MatReader('../data/dataset_EQ')
indices = scipy.io.loadmat('../data/train_test_index.mat')

index_train = indices['train'].T
index_test = indices['test'].T

x_train = reader.read_field('ground_motion')[index_train[:,0]-1,21::]
y_train = reader.read_field('displacement')[index_train[:,0]-1,21::,:]

x_test = reader.read_field('ground_motion')[index_test[:,0]-1,21::]
y_test = reader.read_field('displacement')[index_test[:,0]-1,21::,:]
...
y_train = y_train.reshape(ntrain,s,6)
y_test = y_test.reshape(ntest,s,6)
...
for ep in range(epochs):
    ...
    for x, y in train_loader:
        ...
        loss = torch.sum(torch.mean(torch.einsum('ijk,jk->ijk',torch.square(out - y),lamb**2), dim=0))
        loss.backward()
